...
export function EventsShell({ children }: { children: React.ReactNode }) {
  ...
  if (isSignedIn) {
    return (
      <>
        <NavigationProgress />
        <SidebarProvider defaultOpen className="h-full min-h-screen w-full overflow-hidden">
          <AppSidebar />
          <SidebarInset className="flex min-h-screen flex-1 flex-col overflow-hidden">
            <SiteHeader />
            <main className="flex min-h-0 flex-1 flex-col overflow-y-auto">
              <div className="flex flex-1 flex-col p-4 pb-10 md:p-6 md:pb-12">{children}</div>
            </main>
          </SidebarInset>
        </SidebarProvider>
      </>
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-5xl p-4 pb-10 md:p-6 md:pb-12">{children}</main>
    </div>
  )
}
