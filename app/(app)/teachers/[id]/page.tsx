...
import type { AnalyticsRange, Teacher, TeacherAttendanceSummary, TeacherPayload, TimetableSlot } from "@/lib/types"
...
function TeacherDetailContent() {
  ...
  const isOwner = role === "teacher" && me?.teacher_profile_id === teacherId
  ...
  const [form, setForm] = React.useState<TeacherPayload>({
    name: "",
    school_code: "HIS",
  ...
  const loadCore = React.useCallback(async () => {
    ...
    try {
      ...
      const [profile, timetable] = await Promise.all([
  ...
  const openEdit = () => {
    ...
    setForm({
      ...
      join_date: teacher.join_date?.slice(0, 10) ?? null,
  ...
  const saveTeacher = async () => {
    ...
    try {
      ...
      if (!token) throw new Error("No auth token available")
      setTeacher(
        await createApi(token).updateTeacher(teacher.id, {
          ...form,
          name: form.name.trim(),
      ...
      setError(err instanceof ApiError ? err.userMessage : "Failed to update teacher")
    } finally {
      setSaving(false)
    }
  }

  const accountabilityStatus = React.useMemo(
    () =>
      summary?.accountability.by_status
        .filter((entry) => entry.count > 0)
        .map((entry) => ({ ...entry, name: STATUS_LABELS[entry.status] ?? entry.status })) ?? [],
    [summary]
  )
  ...
  const outcomes = React.useMemo(
    () =>
      summary?.personal.by_outcome
        .filter((entry) => entry.count > 0)
        .map((entry) => ({ ...entry, name: OUTCOME_LABELS[entry.outcome] ?? entry.outcome })) ?? [],
    [summary]
  )
  ...
  if (!isSignedIn) {
    return (
      <div className="flex min-h-[40vh] items-center justify-center text-muted-foreground">
        Please sign in to view this teacher.
      </div>
    )
  }
  if (!Number.isFinite(teacherId)) {
    return <div className="container mx-auto py-16 text-center text-muted-foreground">Invalid teacher id.</div>
  }
  if (!isStaff && !isOwner) {
    return (
      <div className="mx-auto max-w-md space-y-4 py-16 text-center">
        <h1 className="text-xl font-semibold">Access denied</h1>
        <p className="text-sm text-muted-foreground">You can only view your own teacher hub.</p>
  ...
  return (
    <StaggerContainer className="container mx-auto max-w-6xl px-4 space-y-6">
      ...
      {loading ? (
        <PageSkeleton blocks={TEACHER_DETAIL_PAGE_LAYOUT} />
      ) : !teacher ? (
        <Empty className="border">
          <EmptyHeader>
            ...
            <EmptyTitle>Teacher not found</EmptyTitle>
            <EmptyDescription>This teacher may have been removed.</EmptyDescription>
          </EmptyHeader>
        ...
        <>
              ...
              <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
                <div className="space-y-3">
                  <div className="flex gap-2">
                    <Badge variant="secondary">{teacher.school_code}</Badge>
                    <Badge variant="outline">
                      {EMPLOYMENT_TYPES.find((item) => item.value === teacher.employment_type)?.label ?? "Unspecified"}
                    </Badge>
                  </div>
                  ...
                  <dl className="grid gap-x-8 gap-y-2 text-sm sm:grid-cols-2">
                    <div>
                      ...
                      <dd className="font-medium">{teacher.unique_code}</dd>
                    </div>
                    <div>
                      <dt className="text-muted-foreground">Joined</dt>
                      <dd className="font-medium">{teacher.join_date ?? "—"}</dd>
                    </div>
                    <div>
                      <dt className="text-muted-foreground">Contact</dt>
                      <dd className="font-medium">{teacher.contact ?? "—"}</dd>
                    </div>
                  </dl>
                </div>
                {isStaff ? (
          ...
          <HubTimetableCard
            slots={slots}
            description="Weekly slots assigned to this teacher — not dated session occurrences."
            slotLine="class"
          />
          <Card className="border-border/80">
            <CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <CardTitle>Teaching accountability</CardTitle>
                <CardDescription>
                  Lesson attendance for sessions this teacher taught (assigned or cover).
                  {summary ? (
              ...
              <Tabs value={range} onValueChange={(value) => setRange(value as AnalyticsRange)}>
                <TabsList>
                  {RANGE_OPTIONS.map((option) => (
                    <TabsTrigger key={option.value} value={option.value} disabled={summaryLoading}>
                      {option.label}
                    </TabsTrigger>
                  ))}
                </TabsList>
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                    {(["present", "late", "absent", "excused"] as const).map((key) => (
                      <div key={key} className="rounded-xl border bg-muted/30 p-3 text-center">
                        <p className="text-xl font-bold">{summary.accountability[key]}</p>
                        <p className="text-[10px] uppercase text-muted-foreground">{STATUS_LABELS[key]}</p>
                      </div>
                    ))}
                  </div>
                  <p className="text-sm text-muted-foreground">
                    {summary.accountability.sessions_taught} sessions taught · {summary.accountability.total_marks}{" "}
                    marks · attended{" "}
                    {summary.accountability.rate_attended == null
                      ? "—"
                      : `${Math.round(summary.accountability.rate_attended * 100)}%`}
                  </p>
                  <TeacherAccountabilityCharts statusData={accountabilityStatus} subjectData={accountabilitySubjects} />
                </div>
              )}
            </CardContent>
          </Card>
          <Card className="border-border/80">
            <CardHeader>
              <CardTitle>Personal attendance</CardTitle>
              <CardDescription>Session outcomes for this teacher.</CardDescription>
            </CardHeader>
            <CardContent>
              {summaryLoading ? (
                <AttendanceOverviewSkeleton />
                ...
                <div className="grid gap-6 lg:grid-cols-[1fr_280px]">
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                    {(["present", "covered", "cover_taught", "no_show", "cancelled", "unmarked"] as const).map((key) => (
                      <div key={key} className="rounded-xl border bg-muted/30 p-4">
                        <p className="text-2xl font-bold">{summary.personal[key]}</p>
                        <p className="text-xs text-muted-foreground">{OUTCOME_LABELS[key]}</p>
                      </div>
                    ))}
                  </div>
                  <TeacherPersonalOutcomeChart outcomes={outcomes} />
                </div>
              ) : null}
            </CardContent>
          </Card>
          <Card className="border-border/80">
            <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <CardDescription>
                  Assigned versus substitute teacher.
                  {isStaff ? " Manage substitutes on Sessions." : ""}
                </CardDescription>
              </div>
              {isStaff ? (
            ...
            <CardContent>
              ...
              ) : summary.personal.recent_sessions.length === 0 ? (
                <p className="text-sm text-muted-foreground">No recent sessions in this range.</p>
              ) : (
                <div className="divide-y rounded-xl border">
                  {summary.personal.recent_sessions.map((session) => {
                    const hasSubstitute = Boolean(session.actual_teacher_id)
                    return (
                      <div
                        key={`${session.kind}-${session.session_id}`}
                        className="flex flex-col gap-2 p-4 sm:flex-row sm:items-center sm:justify-between"
                      >
                        <div className="min-w-0 space-y-1">
                          <div className="flex flex-wrap items-center gap-2">
                            <p className="font-medium">
                              {session.date} · {session.class_label ?? "Ad-hoc"}
                              {session.subject_label ? ` · ${session.subject_label}` : ""}
                            </p>
            ...
            <Dialog open={editOpen} onOpenChange={setEditOpen}>
              <DialogContent onClose={() => setEditOpen(false)}>
                <DialogHeader>
                  ...
                  <DialogDescription>Update profile and employment details.</DialogDescription>
                </DialogHeader>
                <div className="flex flex-col gap-4">
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setForm((item) => ({ ...item, school_code: value ?? "HIS" }))}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        {SCHOOL_CODES.map((option) => (
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setForm((item) => ({
                          ...item,
                          employment_type:
                            value === "none" ? null : (value as TeacherPayload["employment_type"]),
                        }))
                      }
                    >
                      <SelectTrigger>
                  ...
                  <div>
                    <label className="text-sm font-medium" htmlFor="join-date">
                      Join date
                    </label>
                    <Input
                      id="join-date"
                      ...
                      onChange={(event) => setForm((item) => ({ ...item, join_date: event.target.value || null }))}
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      id="contact"
                      value={form.contact ?? ""}
                      onChange={(event) => setForm((item) => ({ ...item, contact: event.target.value }))}
                    />
                  </div>
                </div>
                <DialogFooter>
                  ...
                  <Button disabled={saving || !form.name.trim()} onClick={() => void saveTeacher()}>
